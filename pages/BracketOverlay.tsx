...
const BracketOverlay: React.FC = () => {
  ...
  return (
    <div className="h-screen w-screen bg-transparent text-white font-sans overflow-hidden flex flex-col items-center justify-center p-10 select-none">
        {/* TOURNAMENT NAME BAR */}
        <div className="mb-16 bg-black/80 backdrop-blur-md px-12 py-3 rounded-full border-t-2 border-loud-500 shadow-2xl flex items-center gap-6">
            <h1 className="text-3xl font-black italic uppercase tracking-tighter text-loud-500">{data.name}</h1>
        ...
        <div className="flex gap-16 justify-center items-center scale-90 origin-center">
                            ...
                            return (
                                <div key={m.id} className="relative">
                                    <div className="bg-black/90 border-2 border-white/10 rounded-2xl w-64 overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
                                        {/* Team A */}
                                        <div className={`p-3 flex items-center justify-between border-b border-white/5 transition-all ${m.winnerId === m.teamAId ? 'bg-loud-500/20' : ''}`}>
                                            <div className="flex items-center gap-3 overflow-hidden">
                                                <div className="w-7 h-7 rounded bg-loud-500 flex items-center justify-center font-black text-[10px] text-black italic">
                                                    {teamA?.name.charAt(0) || '?'}
                                                </div>
                                            ...
                                            <span className="text-sm font-black text-loud-500 italic mr-2">{m.scoreA}</span>
                                        </div>

                                        {/* Team B */}
                                        <div className={`p-3 flex items-center justify-between transition-all ${m.winnerId === m.teamBId ? 'bg-loud-500/20' : ''}`}>
                                            <div className="flex items-center gap-3 overflow-hidden">
                                                <div className="w-7 h-7 rounded bg-loud-500 flex items-center justify-center font-black text-[10px] text-black italic">
                                                    {teamB?.name.charAt(0) || '?'}
                                                </div>
                                            ...
                                            <span className="text-sm font-black text-loud-500 italic mr-2">{m.scoreB}</span>
                                        </div>
                                    </div>
                                    
                                    {/* Small round badge */}
                                    <div className="absolute -left-3 top-1/2 -translate-y-1/2 w-6 h-6 bg-loud-500 text-black text-[8px] font-black rounded-full flex items-center justify-center italic border-2 border-black">
                                        {m.id.split('-')[1]}
                                    </div>
                                </div>
                            );
            ...
            <div className="flex flex-col items-center ml-10">
                <div className="w-20 h-20 rounded-full bg-loud-500 flex items-center justify-center text-black mb-4 shadow-[0_0_50px_rgba(234,179,8,0.4)] animate-pulse">
                    <Trophy size={40} />
                </div>
                <div className="bg-black/95 px-10 py-5 rounded-3xl border-2 border-loud-500 text-3xl font-black text-loud-500 uppercase italic shadow-2xl tracking-tighter">
